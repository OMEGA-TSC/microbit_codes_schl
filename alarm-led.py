...
while True:
    ...
    if set == 1:
        if (accelerometer.get_x() > (set_x + hranice))  or (accelerometer.get_x() < (set_x - hranice)):
            encdata = "alarm"
            encdata = enc.bin(encdata)
            radio.send(encdata)
            
        elif (accelerometer.get_y() > (set_y + hranice))  or (accelerometer.get_y() < (set_y - hranice)):
            encdata = "alarm"
            encdata = enc.bin(encdata)
            radio.send(encdata)
            
        elif (accelerometer.get_z() > (set_z + hranice))  or (accelerometer.get_z() < (set_z - hranice)):
            encdata = "alarm"
            encdata = enc.bin(encdata)
            radio.send(encdata)
